...
const Notifications = () => Widget.Button({
  ...
  on_primary_click: () => exec('swaync-client -t'),
  label: '',
});
...
const Clock = () => Widget.Label({
  ...
  setup: self => self
    ...
    .poll(1000, self => execAsync(['date', '+%I:%M%p %b %d'])
      .then(date => self.label = date)),
});

const Storage = () => Widget.Button({
  class_name: 'storage',
  label: Utils.exec('/home/justin/Documents/Scripts/storage.sh'),
  on_primary_click: () => exec("hyprctl dispatch exec [float;move 57% 48%;animation slideup] 'kitty -e ncdu /'"),
});

const Pacman = () => Widget.Button({
  class_name: 'pacman',
  label: Utils.exec('/home/justin/Documents/Scripts/updates.sh'),
  on_primary_click: () => exec("hyprctl dispatch exec [float;move 57% 48%;animation slideup] 'kitty -e yay'"),
});
...
const Bar = (monitor = 0) => {
  ...
  //Utils.interval(1000, () => {
  //  Pacman.label = Utils.exec('/home/justin/Documents/Scripts/updates.sh')
  //});
